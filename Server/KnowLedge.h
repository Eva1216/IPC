...
/*
LPC	(Local Proceduce Call)
端口是一种面线连接的通信机制 通信的双方需要先建立"连接"  一般建立在用户进程之间  3中情况

不带数据的纯报文
不大于256字节的短报文
如果大于256字节的长报文  就要在双方之间建立两个共享内存区(Section) 双方通过共享内存区交换数据 
但通过报文进行协调和同步

Tips:大块数据之所以要通过共享内存区交换，一来是因为这样就为用于Port机制的缓冲区设置了一个上限，便于内存管理。
而更重要的是提高了效率，因为否则便要在发送端把大块数据搬入内核空间，又在接收端把大块数据搬到用户空间。
*/
